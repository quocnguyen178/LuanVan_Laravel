        ...
        <div class="row">
            ...
            <div class="col-lg-7" style="padding-bottom:120px">
                @if(!$errors-> has('matkhaucu') || !$errors-> has('matkhaumoi') || !$errors-> has('nhaplaimatkhau'))
                    ...
                    @endif
                @endif
                <form action="{{route('luanvan.postthem')}}" method="POST">
                	<input type="hidden" name="_token" value="{{ csrf_token() }}" />
                	<div class="form-group">
                        <label>Mã luận văn</label>
                        <input class="form-control" name="malv" readonly="" value="{{$malv}}" placeholder="Nhập mã luận văn" />
                    </div>
                    <div class="form-group">
                        <label>Tên luận văn</label>
                        <input class="form-control" name="tenlv" value="{{old('tenlv')}}" placeholder="Nhập tên luận văn" />
                    </div>
                    <div class="form-group">
                        <label>Loại luận văn</label>
                        <select class="form-control" name = "maloailv" id="loailv">
                            @foreach($loailv as $llv)
                            <option value="{{ $llv -> maloailv }}">{{$llv-> tenloailv}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Tháng năm nộp luận văn</label><small> ( mm-YYYY )</small>
                        <input class="form-control" name="thangnam" id="thangnam" onChange="ktthangnam()" value="{{old('thangnam')}}" placeholder="Nhập tháng năm nộp luận văn" />
                        <span id="thang_error" class="error" style="color: red;"></span>
                        <span id="nam_error" class="error" style="color: red;"></span>
                    </div>
                    <div class="form-group">
                        <label>Màu bìa</label>
                        <input class="form-control" name="maubia" value="{{old('maubia')}}" placeholder="Nhập màu bìa" />
                    </div>
                    <div class="form-group">
