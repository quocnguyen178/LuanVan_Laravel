        ...
        <div class="row">
            ...
            <div class="col-lg-7" style="padding-bottom:120px">
            	@if(!$errors-> has('matkhaucu') || !$errors-> has('matkhaumoi') || !$errors-> has('nhaplaimatkhau'))
                    ...
                    @endif
                @endif
                <form action="{{route('phanloailv.postthem')}}" method="POST">
                	<input type="hidden" name="_token" value="{{ csrf_token() }}" />
                	<div class="form-group">
                        <label>Mã loại luận văn</label>
                        <input class="form-control" name="maloailv" readonly="" value="{{$maloailv}}" placeholder="Nhập mã loại luận văn" />
                    </div>
                    <div class="form-group">
                        <label>Tên loại luận văn</label>
                        <input class="form-control" name="tenloailv" value="{{old('tenloailv')}}" placeholder="Nhập tên loại luận văn" />
                    </div>
